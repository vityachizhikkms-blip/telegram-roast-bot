...
WELCOME_TEXT = (
    ...
    "Пиши по-человечески:\n"
    "Серега как дела — свежая история/отчет по Серому\n"
...
def natural_command(text: str) -> tuple[str | None, str | None]:
    ...
    if any(phrase in normalized for phrase in ("как дела", "че как", "что нового", "что там", "как сам")):
        return "story", None
...
async def startup_broadcast(bot: Bot) -> None:
    ...
    text = (
        "Серый перезапустился и опять пришел нести хуйню.\n\n"
        + WELCOME_TEXT
        + "\n\nНовая фишка: напиши 'Серега как дела' или 'Серега расскажи историю' — "
        "и он выдаст очередной разъебный отчет из своей тупой биографии."
    )
    for row in rows:
        try:
