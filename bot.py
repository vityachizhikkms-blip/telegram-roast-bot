...
async def typing_pause(bot: Bot, chat_id: int, text: str = "") -> None:
    delay = min(6.0, max(1.1, len(text) / 85)) + random.uniform(0.3, 1.2)
    try:
...
def loan_request_text(loan: sqlite3.Row) -> str:
    return (
        f"Я на связи. Займите мне {loan['amount']} ₽.\n\n"
        f"Причина: {loan['reason']}.\n\n"
        "Кто выручит финансового гения? Жмите кнопку, потом будете с меня спрашивать эту хуйню."
    )
...
async def score_message(message: Message, bot: Bot) -> None:
    ...
    if replies_to_bot(message):
        await reply_alive(message, random.choice(SERG_CONFLICT_REPLIES))
        return

    if is_plain_serg_call(text):
        await reply_alive(message, random.choice(DIRECT_SERG_REPLIES))
        return
    ...
    if should_seriy_defend(text):
        ...
        await reply_alive(message, random.choice(SERIY_DEFENSE_REPLIES + SERG_CHATTER + SERG_CONFLICT_REPLIES))
        return

    if mentions_seriy(text) and random.random() < SERG_CHAT_REPLY_CHANCE:
        await reply_alive(message, random.choice(SERG_CHATTER + DIRECT_SERG_REPLIES + FIRST_PERSON_LORE_FACTS))
        return

    if random.random() < 0.035:
        await reply_alive(message, random.choice(SERG_CHATTER))
...
async def idle_roast_loop(bot: Bot) -> None:
    while True:
        ...
        for row in stupidity_rows:
            try:
                await send_alive(bot, row["chat_id"], random.choice(FIRST_PERSON_STUPIDITY))
